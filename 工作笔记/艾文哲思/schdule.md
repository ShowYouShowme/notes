...
- [ ] 完成玩家数据修改
- [ ] 完成配牌
- [ ] 学会使用libcurl，包括windows和linux。这东西编译和安装估计比较麻烦。
- [ ] 读懂TC_Singleton相关的代码实现
- [ ] globe.h 头文件里面的\_\_TRY\_\_ 和__CATCH__ 宏使用提示有问题，Debug跳转不对；宏定义目前只替换单行的，这样debug不会有问题。VS提示也OK。
- [ ] 试图修改\_\_catch\_\_宏定义，否则VS提示错误；可以在catch里面增加一个分号试试看
- [ ] 学习python的类型注解--把https://www.jb51.net/article/166907.htm里面的东西全部整理出来
...
2. 定义订单服务的协议，熟悉流程
3. 对接gm接口
4. 排行榜查不到数据，咨询测试是否需要查询功能
5. 与策划沟通任务的功能
6. libcurl用异步发起请求，同步请求会阻塞
7. 定义协议和数据库的表结构
8. 把各种包和C++第三方库的安装，汇总到一个文件，方便日后查阅